...
class ThirdTermFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_third_term, container, false)
    ...
    private fun retriveMarks() {
        databaseReference.child("users").child(uid).child("results").child("Term3")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(context, "No Marks Updated for Term 3", Toast.LENGTH_LONG).show()
                    }
                }
    ...
    private fun updateMarks() {
        ...
        // 👇 write to database
        databaseReference.child("users").child(uid).child("results").child("Term3").updateChildren(profileMap).addOnSuccessListener {
