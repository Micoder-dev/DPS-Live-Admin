...
class FirstTermFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_first_term, container, false)
    ...
    private fun retriveMarks() {
        databaseReference.child("users").child(uid).child("results").child("Term1")
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    if (snapshot.exists()) {
                        ...
                        Toast.makeText(context, "No Marks Updated for Term 1", Toast.LENGTH_LONG).show()
                    }
                }
    ...
    private fun updateMarks() {
        ...
        // 👇 write to database
        databaseReference.child("users").child(uid).child("results").child("Term1").updateChildren(profileMap).addOnSuccessListener {
